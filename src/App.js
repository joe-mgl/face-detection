...
class App extends Component {
  ...
  calcFaceLoc = (loc) => {
    const clarifaiFace = loc.outputs[0].data.regions[0].region_info.bounding_box;
  ...
  onButtonSubmit = () => {
    this.setState({imageUrl: this.state.input})
      fetch('https://obscure-savannah-52186.herokuapp.com/imageurl',{
        method: 'post',
      ...
      .then(response => {
        if (response) {
          fetch('https://obscure-savannah-52186.herokuapp.com/image',{
            method: 'put',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({
